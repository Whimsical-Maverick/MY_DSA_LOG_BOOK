class Solution {
  public:
    int Solve(vector<int>&arr,int&total,int sum,int idx,int&d,vector<vector<int>>&dp)
    {
        ...
        if(sum<=total)dp[idx][sum]=Solve(arr,total,sum+arr[idx],idx-1,d,dp) + Solve(arr,total,sum,idx-1,d,dp);
        return Solve(arr,total,sum+arr[idx],idx-1,d,dp) + Solve(arr,total,sum,idx-1,d,dp);
    }
